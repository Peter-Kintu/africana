...
class QuestionSerializer(serializers.ModelSerializer):
    # Ensure lesson_uuid is handled correctly for input/output
    ...
    class Meta:
        model = Question
        fields = '__all__'
        read_only_fields = ('created_at', 'updated_at', 'ai_generated_feedback') # AI feedback is generated by backend

    def create(self, validated_data):
        lesson_uuid = validated_data.pop('lesson')['uuid'] # Extract lesson_uuid
        try:
    ...
    def update(self, instance, validated_data):
        if 'lesson' in validated_data:
            lesson_uuid = validated_data.pop('lesson')['uuid']
            try:
                lesson = Lesson.objects.get(uuid=lesson_uuid)
...
class QuizAttemptSerializer(serializers.ModelSerializer):
    ...
    # This tells DRF to accept the UUID string from the client.
    ...
    class Meta:
        model = QuizAttempt
        fields = '__all__' # Use '__all__' now that 'uuid' is explicitly defined
        read_only_fields = ('synced_at', 'sync_status', 'ai_feedback_text', 'raw_ai_response') # These are managed by backend

    def create(self, validated_data):
        student_id_code = validated_data.pop('student_id_code', None)
        question_uuid = validated_data.pop('question')['uuid']
        ...
        if student_id_code:
            ...
            # Fallback to current authenticated user's student profile if no student_id_code
            try:
        ...
        # Ensure the UUID from Flutter is used for creation
        quiz_attempt_uuid = validated_data.pop('uuid') # Pop the UUID field
        
        # Create the QuizAttempt instance with the provided UUID
